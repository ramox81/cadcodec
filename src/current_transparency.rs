...
impl CadDocument {
    ...
    pub fn current_entity_transparency(&self) -> Transparency {
        let read = || {
            ...
            let ObjectType::DictionaryVariable(value) = self.objects.get(&variables.get(CURRENT)?)? else { return None; };
            Some(Transparency::from_alpha_value(value.value.parse::<u32>().ok()?))
        };
